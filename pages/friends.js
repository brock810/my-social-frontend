...
const FriendsPage = ({ friendsList: initialFriendsList }) => {
  ...
  const fetchData = async () => {
    try {
      const responseUser = await fetch('http://localhost:3001/api/getUser');
      ...
      const friendsResponse = await fetch('http://localhost:3001/api/getFriends');
  ...
  const handleDeleteFriend = async (friendId) => {
    try {
      const response = await fetch(`http://localhost:3001/api/deleteFriend/${friendId}`, {
        method: 'DELETE',
      });
