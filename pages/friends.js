...
const FriendsPage = ({ friendsList: initialFriendsList }) => {
  ...
  const fetchData = async () => {
    try {
      const responseUser = await fetch('http://localhost:80/api/getUser');
      ...
      const friendsResponse = await fetch('http://localhost:80/api/getFriends');
  ...
  const handleDeleteFriend = async (friendId) => {
    try {
      const response = await fetch(`http://localhost:80/api/deleteFriend/${friendId}`, {
        method: 'DELETE',
      });
  ...
  const handleAddFriend = async () => {
    try {
      const response = await fetch('http://localhost:80/api/addFriend', {
        method: 'POST',
        headers: {
